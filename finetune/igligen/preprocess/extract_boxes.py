...
import bisect
import torch
from GroundingDINO.groundingdino.util.inference import load_model, preprocess_caption, annotate
    ...
    def __iter__(self):
        for tar_file in self.tar_files:
            ...
            for image_path, image in content:
                ...
                index = int(os.path.splitext(filename)[0].replace("image_", ""))
                try:
...
all_captions = pd.read_csv("image_caption.csv")

keys = all_captions["key"].str.replace("image_", "").astype(int).tolist()

model = load_model("GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py", "GroundingDINO/weights/groundingdino_swint_ogc.pth")
model = model.to("cuda")

dataloader = torch.utils.data.DataLoader(dataset, batch_size=None, num_workers=0, pin_memory=True)
run(dataloader)
